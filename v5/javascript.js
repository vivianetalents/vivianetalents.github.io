    ...
    langOptions.forEach(option => {
        option.addEventListener('click', () => {
            ...
            if (lang && window.I18N) {
                ...
                const img = option.querySelector('.flag-icon');
                const code = option.querySelector('.lang-code');
                if (img && code) {
                    const btnImg = langBtn.querySelector('.flag-icon');
                    const btnCode = langBtn.querySelector('.lang-code');
                    btnImg.src = img.src;
                    btnCode.textContent = code.textContent;
                }
            }
            langDropdown.hidden = true;
            langBtn.setAttribute('aria-expanded', 'false');
